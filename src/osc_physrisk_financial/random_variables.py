...
class DiscreteRandomVariable(RandomVariable):
    ...
    def __init__(
        ...
        if intervals is not None:
            if convert_to_osc_format:
                ...
                if not np.isclose(total_prob,1):
                    ...
                    if 0 in intervals:
                        ...
                        probabilities[zero_index] += 1 - total_prob
                    else:
                        intervals = np.insert(intervals, 0, 0)
                        probabilities = np.insert(probabilities, 0, 1 - total_prob)
                ...
                if len(self.intervals) != len(probabilities) + 1:
                    raise ValueError(
            ...
            self.values = np.array(values)[sorted_indices]
            self.probabilities = np.array(probabilities)[sorted_indices]
    ...
    def __mul__(self, other: Union[float, int]):
        ...
        if isinstance(other, (int, float)):
            ...
            return DiscreteRandomVariable(
                values=scaled_values, probabilities=self.probabilities
            )
        else:
    ...
    def __add__(self, other: Union[float, int]):
        ...
        if isinstance(other, (int, float)):
            ...
            return DiscreteRandomVariable(
                values=shifted_values, probabilities=self.probabilities
            )
        else:
            return NotImplemented
    ...
    def check_values(self, min_value: float = 0, max_value: float = 1) -> bool:
        ...
        """
        return np.all((min_value <= self.values) & (self.values <= max_value))
